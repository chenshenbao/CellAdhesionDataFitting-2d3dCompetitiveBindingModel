...
m3o = kf3o*c*acr*mr0/(kf3o*c+kr3o); % psgl-1 number decreasing by soluble L-selectin competitive binding
mr = mr0-m3o./acr;                             % psgl-1 density on RBC after shedding
ml = ml0*(r0+(1-r0)*exp(-ks*T));               % L-selectin density on T cell after shedding

m = ceil(min(ac*mr,c*V*NA)) ;                  % 3d bonds number upper limit
n = ceil(ac*min(mr,ml));                       % 2d bonds number upper limit

% initial condition 
p0 = zeros((m+1)*(n+1),1);
p0(1)=1;                                       % no bonds at initial condition

% ode45 solving equations
[t,p]=ode45('dpnm',tspan,p0);

% cell adhesion probability (sum(Pmn,n¡Ù0))
...
saveas(gcf,'23D_entire_noshedding');
x = get(h1,'xdata');
y = get(h1,'ydata');
z = [x',y'];
fname = './23D_entire_noshedding.txt';
